    ...
    return (
        <>
            ...
            <div className={`fixed inset-0 z-[100] bg-black/60 backdrop-blur flex flex-col items-center justify-center text-lg gap-8 lg:hidden transition-transform duration-400 ${isMenuOpen ? "translate-x-0" : "-translate-x-full"}`}>
                ...
                {user && <Link to="/my-portfolios" className="text-white hover:text-indigo-400 transition" onClick={() => setIsMenuOpen(false)}>My Portfolios</Link>}
                <button onClick={() => { setIsMenuOpen(false); openFeature('/generate', 'CareerFolio'); }} className="text-white hover:text-indigo-400 transition">Generate Portfolio</button>
                <button onClick={() => { setIsMenuOpen(false); openFeature('/career-bot', 'AI Career Coach'); }} className="flex items-center gap-2 text-white hover:text-purple-400 transition">
                    <MessageSquare className="size-5" />
                    AI Career Coach
                </button>
                <button onClick={() => { setIsMenuOpen(false); openFeature('/jd-readiness', 'JD Readiness'); }} className="flex items-center gap-2 text-white hover:text-cyan-300 transition">
